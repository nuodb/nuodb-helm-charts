...
Using the NuoDB database helm chart, set in values file the following values for nuocollector.* section.

Refer to Sumo Logic Collectors to harvest the Sumo Logic endpoint to insert into the yaml snippet below.
...
Sumo Logic Collectors for NuoDB Insight data.

NuoDB Insights graphs in Sumo Logic dashboards.

Kubernetes Graphs in Sumo Logic (Optional)
...
Tip: Too much logging? Control NuoDB logs by `logging.consoleLogLevels` 
