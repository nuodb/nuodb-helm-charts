...
func TestDatabaseSecretsDefault(t *testing.T) {
	...
	for _, obj := range testlib.SplitAndRenderSecret(t, output, 1) {
		...
		assert.Equal(t, expectedPassword, obj.StringData["database-password"])
		assert.NotEmpty(t, obj.StringData["database-username"])
	}

	// Try again with a random password
	options.SetValues["database.generatePassword.enabled"] = "true"
	output = helm.RenderTemplate(t, options, helmChartPath, "release-name", []string{"templates/secret.yaml"})
...
func TestDatabaseTolerations(t *testing.T) {
	// Path to the helm chart we will test
	helmChartPath := testlib.DATABASE_HELM_CHART_PATH

	options := &helm.Options{
		SetValues: map[string]string{
			"database.te.tolerations[0].key":   "te-tol-0",
			"database.te.tolerations[0].value": "te-tol-0-value",
			"database.te.tolerations[1].key":   "te-tol-1",
			"database.te.tolerations[1].value": "te-tol-1-value",
			"database.sm.tolerations[0].key":   "sm-tol-0",
			"database.sm.tolerations[0].value": "sm-tol-0-value",
			"database.sm.tolerations[1].key":   "sm-tol-1",
			"database.sm.tolerations[1].value": "sm-tol-1-value",
		},
	}
...
func TestLogVolumeSizing(t *testing.T) {
	// Path to the helm chart we will test
	helmChartPath := testlib.DATABASE_HELM_CHART_PATH

	checkLogVolumeSize := func(t *testing.T, claims []corev1.PersistentVolumeClaim, size string) {
		var logVolume *corev1.PersistentVolumeClaim
		for _, volume := range claims {
			// Volume name is either log-volume or release-name-nuodb-cluster0-demo-database-log-te-volume
			match, err := regexp.MatchString("log.*-volume", volume.Name)
		...
		require.NotNil(t, logVolume, "Expected to find log-volume")
	...
	t.Run("fromExplicitValue", func(t *testing.T) {
		options := &helm.Options{
			SetValues: map[string]string{
				"database.sm.logPersistence.enabled":  "true",
				"database.te.logPersistence.enabled":  "true",
				"database.sm.logPersistence.size":     "1Gi",
				"database.sm.resources.limits.memory": "2Gi",
				"database.te.logPersistence.size":     "4Gi",
				"database.te.resources.limits.memory": "5Gi",
			},
		}
		...
		t.Run("testTe", func(t *testing.T) {
			output := helm.RenderTemplate(t, options, helmChartPath, "release-name", []string{"templates/persistentvolumeclaim.yaml"})
			checkLogVolumeSize(t, testlib.SplitAndRenderPersistentVolumeClaim(t, output, 1), "4Gi")
		})
	})

	t.Run("forceSizeToMemory", func(t *testing.T) {
		options := &helm.Options{
			SetValues: map[string]string{
				"database.sm.logPersistence.enabled":      "true",
				"database.te.logPersistence.enabled":      "true",
				"database.sm.logPersistence.sizeToMemory": "true",
				"database.te.logPersistence.sizeToMemory": "true",
				"database.sm.logPersistence.size":         "1Gi",
				"database.sm.resources.limits.memory":     "2Gi",
				"database.te.logPersistence.size":         "4Gi",
				"database.te.resources.limits.memory":     "5Gi",
		...
		t.Run("testSm", func(t *testing.T) {
			...
			for _, obj := range testlib.SplitAndRenderStatefulSet(t, output, 2) {
				...
				checkLogVolumeSize(t, claims, "3Gi")
			}
		})

		t.Run("testTe", func(t *testing.T) {
			output := helm.RenderTemplate(t, options, helmChartPath, "release-name", []string{"templates/persistentvolumeclaim.yaml"})
			checkLogVolumeSize(t, testlib.SplitAndRenderPersistentVolumeClaim(t, output, 1), "7.5Gi")
		})
	})
	...
	t.Run("fromLimit", func(t *testing.T) {
		options := &helm.Options{
			SetValues: map[string]string{
				"database.sm.logPersistence.enabled":  "true",
				"database.te.logPersistence.enabled":  "true",
				"database.sm.resources.limits.memory": "1Gi",
				"database.te.resources.limits.memory": "3Gi",
			},
		}
	...
	t.Run("fromDefaults", func(t *testing.T) {
		options := &helm.Options{
			SetValues: map[string]string{
				"database.sm.logPersistence.enabled": "true",
				"database.te.logPersistence.enabled": "true",
				"database.sm.resources":              "null",
				"database.te.resources":              "null",
			},
		}
		...
		t.Run("testSm", func(t *testing.T) {
			...
			for _, obj := range testlib.SplitAndRenderStatefulSet(t, output, 2) {
				...
				checkLogVolumeSize(t, claims, "60Gi")
			}
		})
	...
	t.Run("testMinimun", func(t *testing.T) {
		options := &helm.Options{
			SetValues: map[string]string{
				"database.sm.logPersistence.enabled":  "true",
				"database.te.logPersistence.enabled":  "true",
				"database.sm.resources.limits.memory": "50.0E7", // 500M
				"database.te.resources.limits.memory": "123Mi",
			},
		}
