...
func runTestDataMigration(t *testing.T, source, target databaseParams) {
	...
	databaseRelease := testlib.StartDatabase(t, namespaceName, admin0, &helm.Options{
		SetValues: map[string]string{
			...
			"database.archiveType":                        source.archiveType,
			"database.sm.resources.requests.cpu":          "250m",
			"database.sm.resources.requests.memory":       "250Mi",
			"database.te.resources.requests.cpu":          "250m",
			"database.te.resources.requests.memory":       "250Mi",
			"database.sm.noHotCopy.journalPath.enabled":   strconv.FormatBool(source.separateJournal),
			"database.sm.noHotCopy.replicas":              "2",
			"database.sm.hotCopy.replicas":                "0",
			"database.securityContext.enabled":            strconv.FormatBool(source.runAsUser != ""),
			"database.securityContext.runAsUser":          source.runAsUser,
	...
	// Pause archiving on an SM and write some more data. This verifies that
	// the journal is migrated correctly.
	kubectlOptions := k8s.NewKubectlOptions("", "", namespaceName)
	output, err = k8s.RunKubectlAndGetOutputE(
		t, kubectlOptions, "exec", "sm-"+fullnameOverride+"-0", "-c", "engine", "--", "sh", "-c",
		"pgrep -x nuodb -a | sed 's/.* sid:\\([^ ]*\\) .*/\\1/' | xargs nuocmd pause archiving --start-id")
	require.NoError(t, err, output)
	output, err = testlib.RunSQLAsUser(
		t, namespaceName, admin0, source.dbName, source.dbaUser, source.dbaPassword,
		"INSERT INTO testtbl (id) values (456)")
	require.NoError(t, err, output)
	...
	for i := range [2]struct{}{} {
		...
		// Suspend SM to prevent any writes to the archive generated by
		// shutdown, which seems to unpause archiving
		output, err = k8s.RunKubectlAndGetOutputE(
			t, kubectlOptions, "exec", smPod, "-c", "engine", "--", "sh", "-c",
			"pgrep -x nuodb | xargs kill -STOP")
		require.NoError(t, err, output)

		// If source and target user IDs are different, make archive
		// directory group-writeable to simulate fsGroup behavior, which
		// is not enabled for hostpath CSI driver
		if source.runAsUser != target.runAsUser {
			output, err = k8s.RunKubectlAndGetOutputE(
				t, kubectlOptions, "exec", smPod, "-c", "engine", "--",
				"chmod", "-R", "g+rwxs", "/var/opt/nuodb/archive/nuodb")
			require.NoError(t, err, output)
			// Do the same with journal directory if it is separate
			if source.separateJournal {
				output, err = k8s.RunKubectlAndGetOutputE(
					t, kubectlOptions, "exec", smPod, "-c", "engine", "--",
					"chmod", "-R", "g+rwxs", "/var/opt/nuodb/journal/nuodb")
				require.NoError(t, err, output)
			}
		}
	}
	...
	output, err = k8s.RunKubectlAndGetOutputE(
		...
		"nuocmd", "check", "database", "--db-name", source.dbName, "--num-processes", "0", "--timeout", "60")
	require.NoError(t, err, output)
	...
	options := &helm.Options{
		SetValues: map[string]string{
			...
			"database.archiveType":                        target.archiveType,
			"database.sm.resources.requests.cpu":          "250m",
			"database.sm.resources.requests.memory":       "250Mi",
			"database.te.resources.requests.cpu":          "250m",
			"database.te.resources.requests.memory":       "250Mi",
			"database.sm.noHotCopy.journalPath.enabled":   strconv.FormatBool(target.separateJournal),
			"database.sm.noHotCopy.replicas":              "2",
			"database.sm.hotCopy.replicas":                "0",
			"database.dataMigration.enabled":              "true",
			"database.initContainers.runInitDisk":         "false",
			"database.securityContext.enabled":            strconv.FormatBool(target.runAsUser != ""),
			"database.securityContext.runAsUser":          target.runAsUser,
			"database.securityContext.runAsNonRootGroup":  "true",
			"database.securityContext.enabledOnContainer": "true",
		},
	}
	// If source has separate journal and target does not, expect error on start-up
	if source.separateJournal && !target.separateJournal {
		databaseRelease = testlib.StartDatabaseNoWait(t, namespaceName, admin0, options)
		for smPod, _ := range sourceArchiveObjects {
	...
	for smPod, sourceArchiveObject := range sourceArchiveObjects {
		...
		if source.dbName != target.dbName {
			require.Contains(t, output, fmt.Sprintf("Relocating archive directory from /var/opt/nuodb/archive/nuodb/%s to /var/opt/nuodb/archive/nuodb/%s", source.dbName, target.dbName))
		}
		if target.separateJournal && (source.dbName != target.dbName || !source.separateJournal) {
			journalDir := "/var/opt/nuodb/journal/nuodb/" + source.dbName
			if !source.separateJournal {
				journalDir = "/var/opt/nuodb/archive/nuodb/" + target.dbName + "/journal"
			}
			require.Contains(t, output, fmt.Sprintf("Relocating journal directory from %s to /var/opt/nuodb/journal/nuodb/%s", journalDir, target.dbName))
		}
		if source.runAsUser != target.runAsUser {
			mc := "1.atm"
			if target.archiveType == "lsa" {
				mc = "1.cat"
			}
			require.Contains(t, output, fmt.Sprintf("Changing ownership of /var/opt/nuodb/archive/nuodb/%s/%s", target.dbName, mc))
		}
	}

	t.Run("restartDatabase", func(t *testing.T) {
		// Write more data
		output, err = testlib.RunSQLAsUser(t, namespaceName, admin0, target.dbName, target.dbaUser, target.dbaPassword, "INSERT INTO testtbl (id) values (789)")
		require.NoError(t, err, output)
		...
		require.True(t, strings.Contains(output, "123"))
		require.True(t, strings.Contains(output, "456"))
		require.True(t, strings.Contains(output, "789"))
	})
}
...
func TestDataMigrationSeparateJournal(t *testing.T) {
	runTestDataMigration(
		t, databaseParams{
			dbName:          "webapp",
			dbaUser:         "dba",
			dbaPassword:     "dba",
			separateJournal: true,
			runAsUser:       "994",
		}, databaseParams{
			dbName:          "target",
			dbaUser:         "newuser",
...
func TestDataMigrationSeparateJournalLsa(t *testing.T) {
	runTestDataMigration(
		t, databaseParams{
			dbName:          "webapp",
			dbaUser:         "dba",
...
func TestDataMigrationMoveJournal(t *testing.T) {
	runTestDataMigration(
		t, databaseParams{
			dbName:      "webapp",
			dbaUser:     "dba",
...
func TestDataMigrationMissingJournal(t *testing.T) {
	// Negative test that specifies a separate journal in the source
	// database but not in the target, which means that the journal will be
	// missing. The prepare-archive script should detect this and prevent
	// data loss by refusing to allow the SM to start. In a real deployment,
	// this will appear as a failed upgrade that will have to be retried
	// with the correct configuration for the target service version.
	runTestDataMigration(
		t, databaseParams{
			dbName:          "webapp",
			dbaUser:         "dba",
