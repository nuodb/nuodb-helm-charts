...
func verifyKillAndInfoInLog(t *testing.T, namespaceName string, adminPodName string, podName string) {
	...
	buf, err := os.ReadFile(oldPodLogFileName)
	...
	output, err := k8s.RunKubectlAndGetOutputE(t, kubectlOptions, "exec", podName, "-c", "engine", "--", "find", "/var/log/nuodb/", "-type", "d")
...
func TestPermanentLossOfAdmin(t *testing.T) {
	...
	k8s.RunKubectl(t, kubectlOptions, "exec", adminToKill, "-c", "admin", "--", "rm", "-rf", "/var/opt/nuodb/admin*")
...
func TestGetDiagnoseInfoAdminLogPersistence(t *testing.T) {
	...
	k8s.RunKubectl(t, options, "exec", admin, "-c", "admin", "--", "mkdir", "/var/log/nuodb/lost+found")
	k8s.RunKubectl(t, options, "exec", admin, "-c", "admin", "--", "chmod", "-rwx", "/var/log/nuodb/lost+found")
	output, err := k8s.RunKubectlAndGetOutputE(t, options, "exec", admin, "-c", "admin", "--", "stat", "-c", "%a", "/var/log/nuodb/lost+found")
	require.NoError(t, err, output)
	...
	// failed
	output, _ = k8s.RunKubectlAndGetOutputE(t, options, "exec", admin, "-c", "admin", "--", "nuocmd", "get", "diagnose-info", "--include-cores", "--output-dir", "/tmp")
